...
pred_gex <- read_delim(
  "data/metsim_prediction/COPDGene_metsim_metaboxcan_predict.txt")
...
n_distinct(metab_ids)
  # 789 without pop
indv_ids <- intersect(metab_mod$sid, pred_gex_mod$ID)  # 957
n_distinct(indv_ids)

metab_mod_filt <- metab_mod %>%
  dplyr::filter(sid %in% indv_ids) %>%
  dplyr::select(sid, all_of(metab_ids)) %>% # 957 by 791 (with pop and id col)
  dplyr::rename(ID = sid)

pred_gex_mod_filt <- pred_gex_mod %>%
  dplyr::filter(ID %in% indv_ids) %>%
  dplyr::select(ID, all_of(metab_ids))  # 957 by 791
...
n_distinct(merge_mod_m$ID)  # 489 males
merge_mod_f <- merge_mod %>%
  dplyr::filter(gender == 2)
n_distinct(merge_mod_f$ID)  # 468 females
...
write_tsv(
  merge, "data/metsim_prediction/pred_v_obs_metsim.txt")
